...
public class SwipeableV2State<T>(
  initialValue: T,
  public val animationSpec: AnimationSpec<Float> = SwipeableV2Defaults.AnimationSpec,
  public val confirmValueChange: (newValue: T) -> Boolean = { true },
  public val positionalThreshold: Density.(totalDistance: Float) -> Float =
    SwipeableV2Defaults.PositionalThreshold,
  public val velocityThreshold: Dp = SwipeableV2Defaults.VelocityThreshold,
) {
  ...
  public val swipeDraggableState: DraggableState = object : DraggableState {
  ...
  private var offset: Float? by mutableStateOf(null)
   ...
   */
  public fun requireOffset(): Float = checkNotNull(offset) {
   ...
   */
  internal val minOffset by derivedStateOf { anchors.minOrNull() ?: Float.NEGATIVE_INFINITY }
  ...
  public var anchors: Map<T, Float> by mutableStateOf(emptyMap())
    ...
    if (targetOffset != null) {
      try {
        swipe {
          ...
          var prev = offset ?: 0f
          animate(prev, targetOffset, velocity, animSpec) { value, velocity ->
  ...
  public suspend fun settle(velocity: Float) {
    ...
    if (confirmValueChange(targetValue)) {
      val settlingAnimation = if (previousValue != targetValue) {
        animationSpec
      } else {
        tween()
      }
      animateTo(targetValue, velocity, settlingAnimation)
    } else {
      // If the user vetoed the state change, rollback to the previous state.
      animateTo(previousValue, velocity, animationSpec)
    }
  }
  ...
  public companion object {
    ...
    ): Saver<SwipeableV2State<T>, T> = Saver(
...
public object SwipeableV2Defaults {
   ...
   */
  public val AnimationSpec: AnimationSpec<Float> = SpringSpec()
